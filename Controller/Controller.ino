...
void dumpMouse(ControllerPtr ctl) {
    USBSerial.printf("idx=%d, buttons: 0x%04x, scrollWheel=0x%04x, delta X: %4d, delta Y: %4d\n",
                   ctl->index(),        // Controller Index
                   ctl->buttons(),      // bitmask of pressed buttons
...
void dumpKeyboard(ControllerPtr ctl) {
    ...
    USBSerial.printf("idx=%d, Pressed keys: ", ctl->index());
    for (int key = Keyboard_A; key <= Keyboard_UpArrow; key++) {
        if (ctl->isKeyPressed(static_cast<KeyboardKey>(key))) {
            const char* keyName = key_names[key-4];
            USBSerial.printf("%s,", keyName);
       }
    }
...
void dumpBalanceBoard(ControllerPtr ctl) {
    USBSerial.printf("idx=%d,  TL=%u, TR=%u, BL=%u, BR=%u, temperature=%d\n",
                   ctl->index(),        // Controller Index
...
void processKeyboard(ControllerPtr ctl) {
    ...
    if (ctl->isKeyPressed(Keyboard_A)) {
        // Do Something
        USBSerial.println("Key 'A' pressed");
    }
    ...
    if (ctl->isKeyPressed(Keyboard_LeftShift)) {
        // Do something else
        USBSerial.println("Key 'LEFT SHIFT' pressed");
    }
    ...
    if (ctl->isKeyPressed(Keyboard_LeftArrow)) {
        // Do something else
        USBSerial.println("Key 'Left Arrow' pressed");
    }
